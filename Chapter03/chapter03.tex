...
\begin{itemize}
    \item \marginnote{8/8:}In Chapter \ref{chr:2}, the natural numbers were defined axiomatically --- we assumed an object 0, an increment operation, and five axioms. As previously noted, this is quite different from the intuitive conceptualization of the natural numbers as measuring how many elements are in a set. More formally\dots
    \begin{itemize}
        \item Chapter \ref{chr:2} treats natural numbers as \textbf{ordinals}: \dq{The ordinals are First, Second, Third, \dots, and are used to order a sequence of objects}{68}
        \item Intuitively, natural numbers are treated as \textbf{cardinals}: \dq{The cardinals are One, Two, Three, \dots, and are used to count how many things there are in a set}{68}
    ...
    \item \marginnote{8/9:}We first define a rigorous basis for two sets having the same number of elements. \cite{bib:AnalysisI} intutively rationalizes Cantor's one-to-one matching principle by saying \dq{Indeed, this is how we first learn to count a set: we correspond the set we are trying to count with another set, such as a set of fingers on your hand}{68}
    \begin{dfn}[Equal cardinality]\label{dfn:cardinalityEqual}
    ...
    \item Note that \dq{the fact that two sets have equal cardinality does not preclude one of the sets from containing the other}{69}
    \begin{itemize}
        \item For example, \dq{if $X$ is the set of natural numbers and $Y$ is the set of even natural numbers, then the map $f:X\to Y$ defined by $f(n):=2n$ is a bijection from $X$ to $Y$\dots and so $X$ and $Y$ have equal cardinality, despite $Y$ being a subset of $X$ and seeming intuitively as if it should only have `half' of the elements of $X$}{69}
    \end{itemize}
    ...
    \begin{prp}[Uniqueness of cardinality]\label{prp:cardinalityUniqueness}
    ...
    \begin{lem}\label{lem:cardinalitySetminus}
        Suppose that $n\geq 1$, and $X$ has cardinality $n$. Then $X$ is non-empty, and if $x$ is any element of $X$, then the set $X-\{x\}$ (i.e., $X$ with the element $x$ removed) has cardinality $n-1$\footnote{\dq{Strictly speaking, $n-1$ has not yet been defined in this text. For the purposes of this lemma, we define $n-1$ to be the unique natural number $m$ such that $m\pplus=n$; this $m$ is given by [Exercise \ref{exr:2.2.2}]}{70}}.
        \begin{proof}
    ...
    \end{proof}
    \item \marginnote{8/10:}As previously alluded to, we can now show that $\{0,1,2\}$ and $\{3,4\}$ do not have equal cardinality since $\{0,1,2\}$ has cardinality 3 and $\{3,4\}$ has cardinality 2.
    \item We now rigorously define the distinction between finite and infinite sets.
    \begin{dfn}[Finite sets]\label{dfn:finiteInfinte}
...
\subsection*{Exercises}
\begin{enumerate}[ref={\thesection.\arabic*}]
    \item \label{exr:3.6.1}\marginnote{8/9:}Prove the following proposition.
    \begin{prp}
        ...
        \end{proof}
    \end{prp}
    \item \label{exr:3.6.2}\marginnote{8/12:}Show that a set $X$ has cardinality 0 if and only if $X$ is the empty set.
    \begin{proof}
        We first need a lemma.
    ...
    \begin{prp}[Cardinal arithmetic]
        ...
        \begin{enumerate}[label={\textup{(}\alph*\textup{)}},ref={\theenumi\alph*}]
            ...
            \end{proof}
            \item \label{exr:3.6.4b}\marginnote{\textup{8/12:}}Let $X$ and $Y$ be finite sets. Then $X\cup Y$ is finite and $\#(X\cup Y)\leq\#(X)+\#(Y)$. If in addition $X$ and $Y$ are disjoint (i.e., $X\cap Y=\emptyset$), then $\#(X\cup Y)=\#(X)+\#(Y)$.
            \begin{proof}
                ...
                \end{align*}
            \end{proof}
            \item \label{exr:3.6.4c}\marginnote{\textup{8/13:}}Let $X$ be a finite set, and let $Y$ be a subset of $X$. Then $Y$ is finite, and $\#(Y)\leq\#(X)$. If in addition $Y\neq X$ (i.e., $Y$ is a proper subset of $X$), then we have $\#(Y)<\#(X)$.
            \begin{proof}
